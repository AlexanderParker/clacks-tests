/*
	...
	This test visualises the number of peers each node is connected to as the network discovery progresses. Brighter cells have more peers.
 */
...
encoder.createReadStream().pipe(fs.createWriteStream('peer-discovery-randomnew.gif'));
encoder.start();
	...
	for (var x = 0; x < gridWidth; x++) {	
		for (var y = 0; y < gridHeight; y++) {
			...
			clacksInstance.onMessageQueued(function(peer,payload) {
				results[this.getOptions().port - basePort] = Object.keys(this.getPeers().new).length + Object.keys(this.getPeers().alive).length
				// Prepare gif frame
