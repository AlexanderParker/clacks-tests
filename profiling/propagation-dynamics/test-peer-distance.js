/*
	...
	This test visualises the maximum "distance" between peers as the network discovery progresses. Brighter cells have more distant peers.
 */
...
encoder.createReadStream().pipe(fs.createWriteStream('peer-distance-topnew.gif'));
encoder.start();
	...
	for (var x = 0; x < gridWidth; x++) {	
		for (var y = 0; y < gridHeight; y++) {
			...
			clacksInstance.onMessageQueued(function(peer,payload) {
				var peerDistance = 0,
					newPeers = this.getPeers().new,
					alivePeers = this.getPeers().alive

				// Calculate max distance among all peers for this instance
				for (var id in newPeers) {
					var distance = Math.abs(newPeers[id].port - this.getOptions().port)
					if (distance > peerDistance) peerDistance = distance
				}
				for (var id in alivePeers) {
					var distance = Math.abs(alivePeers[id].port - this.getOptions().port)
					if (distance > peerDistance) peerDistance = distance
				}

				results[this.getOptions().port - basePort] = distance
				// Prepare gif frame
				...
				for (var i = 0; i < gridHeight * gridWidth; i++) {					
					...
					var pixelValue = Math.ceil(results[i] * factor)
